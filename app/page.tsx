...
const DashboardContent: React.FC = () => {
  ...
  const [selectedPigForKit, setSelectedPigForKit] = useState<PigType | null>(null);
  const [boarIdForKit, setBoarIdForKit] = useState<string>("");
  ...
  const loadFromStorage = useCallback((farmId: string) => {
    try {
      ...
      return { rows: [], hutches: [], pigs: [] };
    }
  }, []);

  const saveToStorage = useCallback((farmId: string, data: { rows: any[]; hutches: any[]; pigs: any[] }) => {
    try {
  ...
  const loadData = useCallback(async () => {
    ...
    if (cachedData.rows.length || cachedData.hutches.length || cachedData.pigs.length) {
    ...
    try {
      ...
      const [rowsResponse, hutchesResponse, pigsResponse, alertsResponse, calenderResponse] = await Promise.all([
  ...
  const handlePigsUpdate = useCallback(
    (updatedPigs: PigType[]) => {
      setPigs(updatedPigs);
      if (user?.farm_id) {
        saveToStorage(user.farm_id, { rows, hutches, pigs: updatedPigs });
      }
  ...
  const handleKitAdded = useCallback(() => {
    ...
    if (selectedPigForKit?.pig_id) {
      notifiedPigsRef.current.delete(selectedPigForKit.pig_id);
    }
    setBoarIdForKit("");
  }, [loadData, selectedPigForKit]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-blue-50 to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      ...
      {showAddKitDialog && selectedPigForKit && (
        <AddKitDialog
          pig={selectedPigForKit}
          sowId={selectedPigForKit.pig_id ?? ""}
          boarId={boarIdForKit}
          sowName={selectedPigForKit.name}
          ...
          >
                          ...
                          >
                            <div>
                              ...
                              <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">
                                {hutchNamesConversion(hutches, alert.message)}
                              </p>
                            </div>
            ...
            <TabsContent value="health">
              {isFeatureAvailable("health") ? (
                <HealthTracker pigs={pigs} hutches={hutches ?? []} />
              ) : (
            ...
            <TabsContent value="feeding">
              {isFeatureAvailable("feeding") ? (
                <FeedingSchedule pigs={pigs} hutches={hutches ?? []} />
              ) : (
            ...
            <TabsContent value="reports">
              {isFeatureAvailable("reports") ? (
                <ReportsDashboard hutches={hutches} />
              ) : (
            ...
            <TabsContent value="calender">
              {isFeatureAvailable("calender") ? (
                <Calendar events={events} hutches={hutches} />
              ) : (
           ...
           <Card className="bg-white/60 dark:bg-gray-800/60 backdrop-blur-sm border-white/20 dark:border-gray-700/20 shadow-xl max-w-2xl mx-auto">
              ...
              <CardContent className="space-y-6">
                <p className="text-sm text-gray-600 dark:text-gray-400 text-center">
                  Get started by creating your farm to manage your pigs, hutches, and more.
                </p>
